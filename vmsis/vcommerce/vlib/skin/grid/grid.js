...
function doPostForm(send_to, form_id, url_redirect){
  var form = $('#' + form_id);
  var url = send_to;
...
/*  if (child_grid != "undefined"){
    jsgrid = ParseGridToJson(child_grid.attr('id'));
    form.append("<input type='hidden' name='child_models' value='"+ JSON.stringify(jsgrid) +"'></input>");
  };*/
  ...
  $.ajax({
    ...
    error: function(data) { 
        ...
        $('#temp_erro').append(data.responseText);
    }
  });
  return false;
};
